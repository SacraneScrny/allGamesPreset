...
namespace Sackrany.Actor.Traits.Stats
{
    public class StatHandlerModule : Module
    {
        ...
        readonly Dictionary<int, ExpandedFloat> _stats = new();
        ...
        bool RegisterInternal(int id, float baseValue)
        {
            ...
            _stats[id] = new ExpandedFloat(baseValue);
        ...
        public ExpandedFloat GetStat<T>() where T : IStat
        {
            _stats.TryGetValue(StatRegistry.GetId<T>(), out var v);
            return v;
        }
        public bool TryGetStat<T>(out ExpandedFloat value) where T : IStat
